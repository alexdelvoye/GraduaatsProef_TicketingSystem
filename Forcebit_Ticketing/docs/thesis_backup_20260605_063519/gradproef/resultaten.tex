...
Een belangrijk resultaat is dat de applicatie niet alleen werkt, maar ook een duidelijke structuur heeft. In de backend hebben controllers, services, repositories, domain rules en DTOs elk een eigen verantwoordelijkheid. Daardoor is het meestal duidelijk waar een wijziging thuishoort. Een controller behandelt HTTP, een service voert de workflow uit, een repository haalt data op en een domain rule bewaakt een businessregel.

Die scheiding maakte wijzigingen tijdens het project eenvoudiger. De statusregels konden aangepast worden in de domain rules en service-laag. Bijlagen werden toegevoegd zonder de ticketlogica afhankelijk te maken van concrete bestandsopslag, omdat opslag achter \texttt{IFileStorageService} zit. Ook e-mail blijft apart: \texttt{TicketService} beslist wanneer een notificatie nodig is, terwijl \texttt{EmailService} de providergerichte details afhandelt.
...
De huidige versie blijft een prototype en geen volledige productieomgeving. Bijlagen worden lokaal op de API-server opgeslagen. Dat is voldoende voor ontwikkeling en demo, maar voor productie zou een aparte bestandsserver of object storage beter zijn. Dan kunnen back-ups, toegangsrechten, schaalbaarheid en opslaglimieten apart beheerd worden. De bestaande interface \texttt{IFileStorageService} maakt zo'n vervanging technisch haalbaar.
